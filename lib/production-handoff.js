...
export function buildProductionWorkSnapshot(order, workId = buildProductionWorkId(order?.id)) {
  ...
  const items = order.items.map((item) => ({
    itemId: snapshotText(item?.id, 160),
    sku: snapshotText(item.sku, 160),
    categorySlug: snapshotText(item.categorySlug, 160),
    formatSlug: snapshotText(item.formatSlug, 160),
    configuration: sanitizeConfiguration(item.values),
    color: snapshotText(item.color ?? "", 120, true),
    finish: snapshotText(item.finish ?? "", 120, true),
  ...
  return {
    ...
    workId,
    order: {
      orderId: snapshotText(order.id, 160),
      orderNumber: snapshotText(order.orderNumber, 160)
    },
    items
  };
}
...
export function isProductionEligibleOrder(order) {
  ...
  if (order.metadata?.paymentReview) return false;
...
function sanitizeConfiguration(value) {
  if (!value || typeof value !== "object" || Array.isArray(value) || !Object.keys(value).length) {
    throw productionHandoffError("production_work_invalid_snapshot", "As medidas compradas estao ausentes ou invalidas.");
  }
  return Object.fromEntries(Object.entries(value).map(([key, entry]) => {
    snapshotText(key, 100);
    if (typeof entry === "string") snapshotText(entry, 240, true);
    else if (typeof entry !== "boolean" && !(typeof entry === "number" && Number.isFinite(entry))) {
      throw productionHandoffError("production_work_invalid_snapshot", "As medidas compradas devem ser valores finitos e simples.");
    }
    return [key, entry];
  }));
}

// Reject malformed purchased data rather than silently changing what gets manufactured.
function snapshotText(value, maximum, optional = false) {
  if (typeof value !== "string" || value.length > maximum || (!optional && !value.trim())) {
    throw productionHandoffError("production_work_invalid_snapshot", "O snapshot comprado possui identificadores ou opcoes invalidos.");
  }
  return value;
}
...
function snapshotQuantity(value) {
  if (!Number.isSafeInteger(value) || value < 1 || value > 10000) {
    throw productionHandoffError("production_work_invalid_snapshot", "A quantidade comprada e invalida.");
  }
  return value;
}
