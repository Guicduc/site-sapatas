...
export const familyVisuals = {
  ...
  "sapata-u": {
    ...
    manual: image(
      ...
      "Sequência de instalação da sapata U por encaixe no perfil"
    ),
    usage: image(
      "usage",
      "Uso",
...
const formatFamilyMap = {
  ...
  "sapata-pino": {
    "pino-inserido": "sapata-pino-inserido"
  }
};

const technicalDrawingOnlyCategories = new Set(["sapata-pino"]);
...
export function getCategoryCardImage(categorySlug, fallbackImage) {
  if (technicalDrawingOnlyCategories.has(categorySlug)) return null;
...
export function getConfiguratorVisuals(categorySlug, formatSlug, values) {
  ...
  if (values?.parafuso === true || values?.parafuso === "true") {
    return [];
  }
