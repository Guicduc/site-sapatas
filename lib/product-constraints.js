...
export function validateManufacturingConstraints(format, values = {}) {
  ...
  if (screw) {
    const screwDiameterRaw = values[screw.screwDiameterKey];
    const screwDiameter = Number(
      screwDiameterRaw === "" || screwDiameterRaw === null || screwDiameterRaw === undefined
        ? screw.defaultScrewDiameterMm
        : screwDiameterRaw
    );
    const countersinkDiameter = screwDiameter * Number(screw.countersinkDiameterFactor || 1);
    ...
    if (Number.isFinite(minimumSize) && sizes.some((size) => Number.isFinite(size) && size + 0.0001 < minimumSize)) {
      screwIssues.push(
