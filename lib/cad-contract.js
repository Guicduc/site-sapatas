import { getProductManifestByRoute, resolveManifestVariant } from "./product-registry.js";
...
export const CAD_MODEL_VERSION = {
  ...
  BASE_SCREW_ROUND: "base-screw-round-gh-v1",
  BASE_SCREW_SQUARE: "base-screw-square-gh-v1"
};
...
const SCREW_BASE_TECHNICAL_DEFAULTS = {
  ...BASE_TECHNICAL_DEFAULTS,
  screwCountersinkDiameterFactor: 2,
  minimumRadialWallMm: 3
};
...
const CAD_MODELS = {
  ...
  "sapata-com-parafuso:redonda": {
    modelVersion: CAD_MODEL_VERSION.BASE_SCREW_ROUND,
    sourceGh: "Produtos/Scripts-GH/Sapata_Lisa_Redonda-com parafuso.gh",
    parameterKeys: ["diametro", "alturaBase", "diametroParafuso"],
    technicalDefaults: SCREW_BASE_TECHNICAL_DEFAULTS
  },
  "sapata-com-parafuso:quadrada": {
    modelVersion: CAD_MODEL_VERSION.BASE_SCREW_SQUARE,
  ...
  "sapata-u:u": {
    variants: {
      default: {
        ...
        parameterKeys: ["diametro", "comprimento", "espessura"],
        technicalDefaults: BASE_TECHNICAL_DEFAULTS
      },
      neck: {
...
export function getCadModelForItem(item) {
  const product = getProductManifestByRoute(item?.categorySlug, item?.formatSlug);
  const variant = resolveManifestVariant(product, item?.values);
  if (variant?.cad) {
    return {
      modelVersion: variant.cad.modelVersion,
      sourceGh: variant.cad.script,
      parameterKeys: variant.cad.sliderOrder,
      sliderTransforms: variant.cad.sliderTransforms,
      technicalDefaults: variant.cad.technicalDefaults || {}
    };
  }
