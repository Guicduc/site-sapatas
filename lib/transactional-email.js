import { getPaymentStatusLabel } from "./order-status.js";
import {
  getShipmentEmailContext,
  getShipmentNotificationIdempotencyKey
} from "./shipment-notification-policy.js";
import { siteUrl } from "./site-data.js";
...
export async function sendAccountAccessCodeEmail(email, code) {
  // Local logins expose the OTP in the UI, so they must never send real email.
  if (process.env.NODE_ENV !== "production") {
    return { sent: false, skipped: "development" };
  }
...
export async function sendAccountPasswordChangedEmail(email, event = "alterada") {
  if (!email) return { sent: false, skipped: "missing_email" };
  return sendTransactionalEmail({
    to: email,
    subject: "A senha da sua conta Baseforma foi atualizada",
    html: renderLayout({
      preview: "Atualização de segurança da conta Baseforma",
      title: "Senha atualizada",
      body: `<p style="margin:0;color:#181c1d;font-size:16px;line-height:1.55">Sua senha foi ${escapeHtml(event)}. As sessões anteriores foram encerradas. Se você não reconhece esta ação, entre por código e fale com o atendimento.</p>`
    }),
    idempotencyKey: `account-password-${hashToken(`${email}:${event}:${new Date().toISOString().slice(0, 13)}`)}`,
    eventName: "account_password_changed"
  }, { optional: true });
}
...
export async function notifyPaymentResolved(order, paymentStatus, providerPaymentId = null) {
  ...
  const paymentKey = providerPaymentId || latestPayment?.providerPaymentId || latestPayment?.id || order.id;
  ...
  return sendTransactionalEmail(
    {
      ...
      eventName: `payment_${paymentStatus}`
    },
    { optional: true }
  );
}
...
export async function notifyOrderShipped(order) {
  if (order?.metadata?.fulfillment?.shipment?.status !== "shipped") {
    return { sent: false, skipped: "shipment_not_shipped" };
  }
  ...
  return sendTransactionalEmail(
    {
      ...
      idempotencyKey: getShipmentNotificationIdempotencyKey(order),
  ...
  try {
    ...
    if (optional) {
      console.warn("[transactional-email] network failed", {
        event: message.eventName || "transactional_email",
        outcome: "network_failed"
      });
      return { sent: false, skipped: "network_failed" };
    }
...
function renderShipmentEmail(order) {
  const shipment = getShipmentEmailContext(order);
  const accountUrl = `${getPublicBaseUrl()}/conta`;
  const shipmentDetails = [
    shipment.carrier ? ["Transportadora", shipment.carrier] : null,
    shipment.serviceName ? ["Servico", shipment.serviceName] : null,
    shipment.trackingCode ? ["Codigo de rastreio", shipment.trackingCode] : null,
    shipment.shippedAt ? ["Enviado em", formatDateTime(shipment.shippedAt)] : null
  ].filter(Boolean);

  return renderLayout({
    preview: `Pedido ${shipment.orderNumber} enviado`,
    title: "Pedido enviado",
    body: [
      `<p style="margin:0 0 20px;color:#181c1d;font-size:16px;line-height:1.55">Seu pedido foi expedido pela Baseforma.</p>`,
        ...
        <tbody>
          <tr>
            <td style="padding:16px;border-right:1px solid #d5ddda;vertical-align:top">
              ...
              <p style="margin:0;color:#181c1d;font-size:17px;line-height:1.25;font-weight:700">${escapeHtml(shipment.orderNumber)}</p>
            </td>
            <td style="padding:16px;vertical-align:top">
              <p style="margin:0 0 6px;color:#5a6264;font-size:12px;line-height:1.3">Status</p>
              <p style="margin:0;color:#004d48;font-size:17px;line-height:1.25;font-weight:700">Enviado</p>
            </td>
          </tr>
        </tbody>
      </table>`,
      shipmentDetails.length
        ? `<p style="margin:0 0 8px;color:#181c1d;font-size:14px;line-height:1.4;font-weight:700">Dados da expedicao</p>
          <table style="width:100%;border-collapse:collapse;margin:0 0 24px;border-top:1px solid #d5ddda;border-bottom:1px solid #d5ddda">
            <tbody>${shipmentDetails.map(([label, value]) => `
              <tr>
                <td style="padding:10px 0;color:#5a6264">${escapeHtml(label)}</td>
                <td style="padding:10px 0;text-align:right;color:#181c1d;font-weight:700">${escapeHtml(value)}</td>
              </tr>`).join("")}</tbody>
          </table>`
        : `<p style="margin:0 0 24px;color:#5a6264;font-size:14px;line-height:1.5">O rastreio nao foi informado neste envio. Consulte sua conta ou responda este e-mail se precisar de ajuda.</p>`,
      `<p style="margin:28px 0 18px"><a href="${escapeAttribute(accountUrl)}" style="background:#007a70;color:#fbfcfa;text-decoration:none;padding:13px 18px;border-radius:8px;display:inline-block;font-weight:700">Acompanhar pedido</a></p>`,
