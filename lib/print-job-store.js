...
import { getDatabaseMode, query } from "./db.js";
import {
...
export async function enqueuePrintJob(input) {
  ...
  if (job.origin.source === "site_order") {
    const route = await claimProductionWorkRoute(
      job.origin.sourceId,
      PRODUCTION_WORK_ROUTE.LEGACY_PRINT_QUEUE,
      "print_job_enqueue"
    );
    if (route !== PRODUCTION_WORK_ROUTE.LEGACY_PRINT_QUEUE) {
      throw printJobError(
        "print_job_external_route_conflict",
        "O pedido pertence ao sistema externo e nao pode entrar em print_jobs."
      );
    }
  }
...
export async function syncSiteOrderPrintJobs(orders) {
  ...
  for (const order of Array.isArray(orders) ? orders : []) {
    const orderInputs = buildPrintJobInputsFromOrder(order, {
    ...
    const route = await claimProductionWorkRoute(
      order.id,
      PRODUCTION_WORK_ROUTE.LEGACY_PRINT_QUEUE,
      "admin_print_job_sync"
    );
    if (route !== PRODUCTION_WORK_ROUTE.LEGACY_PRINT_QUEUE) continue;
    inputs.push(...orderInputs);
    for (const input of orderInputs) jobs.push(await enqueuePrintJob(input));
  }
