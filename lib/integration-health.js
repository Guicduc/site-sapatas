import { getInvoiceProvider, hasMercadoPagoInvoiceEndpoint, hasMercadoPagoInvoiceProvider } from "@/lib/invoice-provider";
...
function checkShipping() {
  ...
  return {
    ...
    message: ok
      ? realProvider
        ? "Melhor Envio configurado para cotacao real."
...
function checkInvoice() {
  ...
  const mercadoPagoProvider = hasMercadoPagoInvoiceProvider();
  const endpointConfigured = hasMercadoPagoInvoiceEndpoint();
  const ok = mercadoPagoProvider && hasMercadoPagoCredentials() && endpointConfigured;
  ...
  return {
    ...
    required: false,
    mode: provider,
    message: mercadoPagoProvider
      ? endpointConfigured
        ? "Emissao fiscal via Mercado Pago configurada para chamada de API."
        : "Provider fiscal Mercado Pago ativo; configure MERCADO_PAGO_INVOICE_API_URL quando o endpoint fiscal estiver liberado."
      : provider === "manual"
        ? "Provider fiscal manual legado; use INVOICE_PROVIDER=mercado_pago."
        : "Provider fiscal desconhecido."
  };
}
