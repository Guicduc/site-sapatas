...
import { getInvoiceConfig, INVOICE_PROVIDERS, isSupportedInvoiceProvider } from "@/lib/invoice-config";
...
function checkInvoice() {
  ...
  const mercadoPagoSystem = config.provider === INVOICE_PROVIDERS.MERCADO_PAGO_SYSTEM;
  const ok = supported && config.documentModel === "nfe" && config.environment === "production";
  ...
  return {
    ...
    message: ok
      ...
      : "Fornecedor fiscal nao suportado ou configuracao fiscal incompleta."
  };
}
