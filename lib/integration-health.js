...
import { isSupportedFocusNfeEnvironment } from "@/lib/focus-nfe";
...
async function checkProductionSystem() {
  try {
    ...
    const enabled = health.mode !== "disabled";
    return {
      id: "production_system",
      label: "Sistema externo de producao",
      ok: health.ok,
      required: enabled,
      mode: health.mode,
      message: !enabled
        ? "Integracao desativada; print_jobs continua como ponte operacional."
        : health.mode === "invalid"
          ? "PRODUCTION_SYSTEM_MODE deve ser disabled, shadow ou active."
        : health.ok
          ? `Integracao ${health.mode} com Postgres e credencial dedicada configurados.`
          : "Configure PRODUCTION_SYSTEM_TOKEN e confirme o schema PostgreSQL antes de ativar."
    };
  } catch (error) {
    return {
      id: "production_system",
      ...
      ok: false,
      required: true,
      mode: process.env.PRODUCTION_SYSTEM_MODE || "disabled",
      message: error.message || "Falha ao verificar a integracao de producao."
    };
  }
}
...
function checkInvoice() {
  ...
  const focusNfe = config.provider === INVOICE_PROVIDERS.FOCUS_NFE;
  const fiscalConfigOk = config.documentModel === "nfe"
    && config.issuerCnpj.length === 14
    && config.issuerUf.length === 2
    && config.ncm.replace(/\D/g, "").length === 8
    && /^\d{4}$/.test(config.cfop || config.cfopIntrastate)
    && /^\d{4}$/.test(config.cfop || config.cfopInterstate);
  const environmentOk = !focusNfe || isSupportedFocusNfeEnvironment(config.environment);
  const credentialsOk = !automated || hasInvoiceApiCredentials(config.provider);
  const webhookOk = !focusNfe || Boolean(String(process.env.FOCUS_NFE_WEBHOOK_TOKEN || "").trim());
  const ok = supported && fiscalConfigOk && environmentOk && credentialsOk && webhookOk;
  const missing = [
    !fiscalConfigOk ? "configuracao fiscal (CNPJ, UF, NCM e CFOP)" : "",
    !environmentOk ? "FOCUS_NFE_ENV valido" : "",
    !credentialsOk && focusNfe ? "FOCUS_NFE_TOKEN" : "",
    !webhookOk ? "FOCUS_NFE_WEBHOOK_TOKEN" : ""
  ].filter(Boolean);
  ...
  return {
    ...
    ok,
    required: false,
    mode: focusNfe ? `${config.provider}:${config.environment || "invalido"}` : config.provider,
    message: !supported
      ? "Fornecedor fiscal nao suportado."
      : automated
        ? ok
          ? focusNfe
            ? `Emissao automatica via ${config.providerLabel} configurada em ${config.environment}, inclusive webhook.`
            : `Emissao automatica de NF-e via ${config.providerLabel} configurada.`
          : config.provider === INVOICE_PROVIDERS.FOCUS_NFE
            ? `Emissao automatica incompleta; configure: ${missing.join(", ")}.`
