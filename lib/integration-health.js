import { getGoogleRedirectUri, hasGoogleOAuthConfig } from "@/lib/google-oauth";
...
function checkGoogleOAuth() {
  ...
  return {
    ...
    ok,
    required: false,
    mode: ok ? "enabled" : "disabled",
    redirectUri: getGoogleRedirectUri(),
    message: ok
      ? "OAuth Google configurado para a conta do cliente."
      : "Opcional. Configure GOOGLE_OAUTH_CLIENT_ID e GOOGLE_OAUTH_CLIENT_SECRET para habilitar."
  };
}

function checkInvoice() {
  const provider = String(process.env.INVOICE_PROVIDER || "manual").trim().toLowerCase();
  ...
  return {
    ...
    message: provider === "manual"
      ...
      : "Fornecedor fiscal externo ainda nao esta implementado neste projeto."
  };
}
