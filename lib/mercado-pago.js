...
import { parseMercadoPagoPaymentPayload } from "./commercial-contracts.js";
...
const paymentsSearchEndpoint = "https://api.mercadopago.com/v1/payments/search";
...
export async function createMercadoPagoPreference(order) {
  ...
  const response = await fetch(preferenceEndpoint, {
    ...
    headers: {
      ...
      "X-Idempotency-Key": getMercadoPagoPreferenceIdempotencyKey(order)
    },
    body: JSON.stringify({
      ...
      // A preferencia nao expira no app. O mesmo pedido sempre reutiliza a mesma
      // preferencia pendente, inclusive para meios com liquidacao demorada.
      metadata: {
        order_id: order.id,
...
export function getMercadoPagoPreferenceIdempotencyKey(order) {
  return `preference:${String(order?.id || "missing-order")}`;
}
...
export async function fetchMercadoPagoPayment(paymentId) {
  ...
  return parseMercadoPagoPaymentPayload(data);
}

export async function searchMercadoPagoPaymentsByReference(externalReference) {
  const accessToken = process.env.MERCADO_PAGO_ACCESS_TOKEN;
  ...
  const searchUrl = new URL(paymentsSearchEndpoint);
  searchUrl.searchParams.set("external_reference", String(externalReference));
  ...
  if (!response.ok) {
    const error = new Error(data?.message || "Could not search Mercado Pago payments.");
    error.code = "mercado_pago_payment_search_failed";
    error.details = data;
    throw error;
  }
...
export function isMercadoPagoWebhookConfigured({
  ...
  return nodeEnv !== "production" || Boolean(secret);
}
