...
const DEFAULT_PREFERENCE_TTL_MINUTES = 120;
...
export async function createMercadoPagoPreference(order) {
  ...
  const validity = buildPreferenceValidityWindow();
  const response = await fetch(preferenceEndpoint, {
    ...
    body: JSON.stringify({
      ...
      back_urls: {
        ...
        failure: `${baseUrl}/pedido-confirmado?orderId=${order.id}&payment=failure`,
        pending: `${baseUrl}/pedido-confirmado?orderId=${order.id}&payment=pending`
      },
      auto_return: "approved",
      expires: true,
      expiration_date_from: validity.from,
      expiration_date_to: validity.to,
      metadata: {
        order_id: order.id,
...
function buildPreferenceValidityWindow() {
  const minutes = Number(process.env.MERCADO_PAGO_PREFERENCE_TTL_MINUTES || DEFAULT_PREFERENCE_TTL_MINUTES);
  const ttlMinutes = Number.isFinite(minutes) && minutes >= 15
    ? minutes
    : DEFAULT_PREFERENCE_TTL_MINUTES;
  const from = new Date();
  const to = new Date(from.getTime() + ttlMinutes * 60 * 1000);
