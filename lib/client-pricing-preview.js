...
export function calculateClientPricePreview(format, values, quantity = 1) {
  ...
  const directUnitCostBrl = predictMonotoneProductionCost(model, values);
...
function resolveVariant(format, values = {}) {
  return (format.variants || []).find((variant) => {
    return Object.entries(variant.condition || {}).every(([key, expected]) => {
      return typeof expected === "boolean"
        ? Boolean(values[key]) === expected
        : values[key] === expected;
    });
  });
}
