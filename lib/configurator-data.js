...
export function calculatePriceBreakdown(format, values, quantity = 1, options = {}) {
  ...
  if (!slicedReference) {
    return buildUnavailableSlicedPriceBreakdown(safeQuantity);
  }
...
function buildUnavailableSlicedPriceBreakdown(quantity) {
  return {
    materialGrams: 0,
    materialCostBrl: 0,
    printHours: 0,
    ...
    channelFeeBrl: 0,
    directUnitCostBrl: 0,
    directOrderCostBrl: 0,
    unitPriceBrl: 0,
    totalPriceBrl: 0,
    quantity,
    pricingMode: "unavailable",
    pricingSource: "orca_slicer_missing_reference",
    pricingAvailable: false,
    pricingUnavailableReason: "Sem referencia Orca valida para esta configuracao.",
    assumptions: pricingAssumptions
  };
}
