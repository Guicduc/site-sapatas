...
function calculateSlicedPriceBreakdown(format, reference, quantity) {
  ...
  const volumeMm3 = estimateBboxVolumeMm3(reference.bbox);
...
function estimateBboxVolumeMm3(bbox = {}) {
  return Number(bbox.x || 0) * Number(bbox.y || 0) * Number(bbox.z || 0);
}
