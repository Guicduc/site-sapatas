...
const progressiveDimensionKeys = new Set([
  "diametro",
  "diametroBase",
  "tamanhoBaseX",
  "tamanhoBaseY",
...
function progressiveSlicerUnitCostFloor(candidates, requestedParams, activeKeys, ranges, parameters, progressiveAxisKeys) {
  ...
  const progressiveKeys = activeKeys.filter((key) => progressiveAxisKeys.has(key));
  const contextualKeys = activeKeys.filter((key) => !progressiveAxisKeys.has(key));
  let contextualCandidates = candidates.filter((sample) => {
    return contextualKeys.every((key) => {
      const parameter = parameterByKey.get(key);
      const tolerance = Math.max(0.01, Number(parameter?.step || 0) / 2);
      return Math.abs(Number(sample.params?.[key]) - Number(requestedParams[key])) <= tolerance + 0.0001;
    });
  });

  if (contextualCandidates.length === 0 && contextualKeys.length > 0) {
    const nearestContextDistance = Math.min(
      ...candidates.map((sample) => contextualParameterDistance(sample, requestedParams, contextualKeys, ranges))
    );
    contextualCandidates = candidates.filter((sample) => {
      return contextualParameterDistance(sample, requestedParams, contextualKeys, ranges) <= nearestContextDistance + 0.0001;
    });
  }

  const dominatedSamples = contextualCandidates.filter((sample) => {
    return progressiveKeys.every((key) => {
      const sampleValue = Number(sample.params?.[key]);
      const requestedValue = Number(requestedParams[key]);
      const parameter = parameterByKey.get(key);
      const guardBand = Number(parameter?.step || 0);
      return Number.isFinite(sampleValue) &&
        Number.isFinite(requestedValue) &&
        sampleValue <= requestedValue + guardBand + 0.01;
    });
  });
...
function contextualParameterDistance(sample, requestedParams, keys, ranges) {
  return Math.sqrt(
    keys.reduce((sum, key) => {
      const range = ranges[key];
      const scale = Math.max(1, range.max - range.min);
      return sum + Math.pow((Number(sample.params?.[key]) - Number(requestedParams[key])) / scale, 2);
    }, 0)
  );
}
