...
function calculateSlicedPriceBreakdown(format, values, reference, quantity) {
  ...
  const calibration = buildSlicedCalibration(format, values);
  const referenceVolumeMm3 = Math.max(1, estimateReferencePrintedVolumeMm3(format, reference));
  const scaleFactor = clamp(targetVolumeMm3 / referenceVolumeMm3, 0.05, 20);
  const materialGrams = targetVolumeMm3 * calibration.materialGramsPerMm3;
  const printMinutes = Math.max(1, Math.pow(targetVolumeMm3, 0.82) * calibration.printMinutesPerVolumeFactor);
...
function buildSlicedCalibration(format, values) {
  const categorySlug = format.calibratedPricingCategorySlug || inferCategorySlugFromSku(format);
  const candidates = slicedPricingReferences.filter((reference) => {
    return (
      reference.siteCategorySlug === categorySlug &&
  ...
  const usable = candidates
    .map((reference) => ({
      materialFactor: Number(reference.materialGrams || 0) / Math.max(1, estimateReferencePrintedVolumeMm3(format, reference)),
      printFactor:
        Number(reference.printMinutes || 0) /
        Math.pow(Math.max(1, estimateReferencePrintedVolumeMm3(format, reference)), 0.82)
    }))
    .filter((item) => item.materialFactor > 0 && item.printFactor > 0);

  if (usable.length === 0) {
    return {
      materialGramsPerMm3: (pricingAssumptions.tpuDensityGPerCm3 / 1000) * 0.86,
      printMinutesPerVolumeFactor: 1 / Math.pow(1000, 0.82)
    };
  }

  return {
    materialGramsPerMm3: median(usable.map((item) => item.materialFactor)),
    printMinutesPerVolumeFactor: median(usable.map((item) => item.printFactor))
  };
}
