...
export async function sliceStlWithOrca({ cadFileName, orderNumber, config = getOrcaSlicerConfig() }) {
  ...
  await assertFileExists(stlPath, "orca_stl_not_found", `STL não encontrado em ${stlPath}.`);
  await assertFileExists(config.executablePath, "orca_executable_not_found", "Orca Slicer não encontrado.");
  await fs.mkdir(config.outputDir, { recursive: true });

  const outputDir = path.join(
    /*turbopackIgnore: true*/ config.outputDir,
    sanitizeFilePart(orderNumber || "order")
  );
  ...
  const gcodeText = gcodePath ? await fs.readFile(/*turbopackIgnore: true*/ gcodePath, "utf8") : "";
  const parsed = parseOrcaOutput(`${stdout}\n${stderr}\n${gcodeText}`);

  if (!parsed.materialGrams || !parsed.printMinutes) {
    throw userError(
      "orca_parse_failed",
      "Orca executou, mas não foi possível ler tempo e material do G-code gerado."
    );
  }
...
export function parseOrcaOutput(text) {
  ...
  const materialGrams = materialGramsFromOrcaOutput(source);
...
function materialGramsFromOrcaOutput(source) {
  ...
  return cubicCentimeters > 0 && density > 0
    ? Math.round(cubicCentimeters * density * 1000) / 1000
    : 0;
}
...
function buildOrcaArgs({ stlPath, outputDir, config }) {
  ...
  if (config.loadSettings) {
    ...
    const profilePath = path.join(
      /*turbopackIgnore: true*/ config.profilesDir,
  ...
  args.push("--outputdir", outputDir, "--slice", "0", stlPath);
  return args;
}
...
async function findNewestGcode(outputDir) {
  const entries = await fs.readdir(/*turbopackIgnore: true*/ outputDir, { withFileTypes: true });
