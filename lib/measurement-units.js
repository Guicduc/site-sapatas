...
export function parseMeasurementInput(displayValue, parameter, measurementSystem) {
  ...
  const canonicalValue = parameter.unit === "mm"
    ...
    : numericValue;
  const boundaryTolerance = parameter.unit === "mm"
    && measurementSystem === MEASUREMENT_SYSTEMS.IMPERIAL
    ? (10 ** -INCH_DECIMAL_PLACES * MILLIMETERS_PER_INCH) / 2 + Number.EPSILON
    : 0;
  const boundedCanonicalValue = canonicalValue < parameter.min
    && parameter.min - canonicalValue <= boundaryTolerance
    ? parameter.min
    : canonicalValue > parameter.max
      && canonicalValue - parameter.max <= boundaryTolerance
      ? parameter.max
      : canonicalValue;

  if (boundedCanonicalValue < parameter.min || boundedCanonicalValue > parameter.max) {
    return {
      value: "",
  ...
  return {
    value: trimTrailingZeros(boundedCanonicalValue.toFixed(CANONICAL_DECIMAL_PLACES)),
    error: ""
  };
}
