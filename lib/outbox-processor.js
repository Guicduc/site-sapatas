...
export async function processOutboxBatch({ limit = 10, workerId = "manual", orderId = null, paymentId = null } = {}, dependencies = {}) {
  ...
  for (let index = 0; index < safeLimit; index += 1) {
    const claimed = await claim({ workerId, orderId, paymentId });
    if (!claimed) break;
