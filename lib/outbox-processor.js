...
export async function processOutboxBatch({ limit = 10, workerId = "manual" } = {}, dependencies = {}) {
  ...
  for (let index = 0; index < safeLimit; index += 1) {
    ...
    try {
      ...
      processed.push({
        ...
        error: error.code || "outbox_processing_failed"
      });
    }
  }
...
export async function dispatchOutboxEvent(event) {
  ...
  if (event.type === OUTBOX_EVENT.PAYMENT_CUSTOMER_EMAIL) {
    ...
    return requireEmailDelivery(
      await notifyPaymentResolved(order, event.payload.paymentStatus),
