import { normalizeFocusNfeEnvironment } from "./focus-nfe.js";
...
export function getInvoiceConfig() {
  ...
  const environment = provider === INVOICE_PROVIDERS.FOCUS_NFE
    ? normalizeFocusNfeEnvironment(process.env.FOCUS_NFE_ENV)
    : cleanText(process.env.INVOICE_ENV || "production").toLowerCase();
  ...
  return {
    ...
    pisRatePercent: nonNegativeNumber(process.env.INVOICE_PIS_RATE_PERCENT, 0),
    cofinsRatePercent: nonNegativeNumber(process.env.INVOICE_COFINS_RATE_PERCENT, 0),
    cancellationWindowDays: positiveInteger(process.env.INVOICE_CANCEL_WINDOW_DAYS, 1)
  };
}
