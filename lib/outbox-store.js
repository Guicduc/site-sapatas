...
export async function claimNextOutboxEvent({ workerId, leaseSeconds, orderId = null, paymentId = null, now = new Date() } = {}) {
  ...
  const result = await query(
    `with candidate as (
       ...
       where attempts < max_attempts
         ...
         and ($6::text is null or payload->>'paymentId' = $6 or event_type = 'focus_nfe_invoice')
         and (
     ...
     returning event.*`,
    [timestamp, safeWorkerId, leaseTokenHash, leasedUntil, orderId, paymentId]
  );
...
function sanitizeError(error) {
  return {
    code: sanitizeOutboxErrorCode(error?.code),
    message: "Falha ao processar evento.",
...
export function sanitizeOutboxErrorCode(code) {
  return /^[a-z][a-z0-9_]{0,79}$/.test(String(code || "")) ? code : "outbox_processing_failed";
}
