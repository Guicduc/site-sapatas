...
import { isPayableOrder, ORDER_STATUS, PAYMENT_STATUS } from "@/lib/order-status";
import { notifyInternalPaymentAlert } from "@/lib/transactional-email";
...
export async function listOrdersByAccountId(accountId) {
  if (getStoreMode() === "postgres") {
    await ensurePostgresSchema();
    const result = await query(`select o.*, c.name as customer_name, c.contact as customer_contact, c.email as customer_email from customer_account_orders ao join orders o on o.id = ao.order_id join customers c on c.id = o.customer_id where ao.account_id = $1 and o.metadata->'account'->>'emailVerifiedAt' is not null order by o.created_at desc`, [accountId]);
    return Promise.all(result.rows.map((row) => hydratePostgresOrder(row)));
  }
  const store = await readLocalStore();
...
export async function consumeAccountRateLimit(key, limit = 5, windowSeconds = 900) {
  ...
  if (!item || now - new Date(item.windowStartedAt).getTime() > windowSeconds * 1000) item = { key, attempts: 0, windowStartedAt: new Date().toISOString() }, store.accountRateLimits.push(item);
  item.attempts += 1;
  await writeLocalStore(store);
...
export async function cancelSupersededOrder(orderId, { reason = "superseded_by_new_checkout" } = {}) {
  ...
  if (!isPayableOrder(order.status) || order.paymentStatus === PAYMENT_STATUS.APPROVED) {
    return order;
  }
...
export async function recordMercadoPagoUpdate({ orderId, preferenceId, paymentId, status, amountBrl, raw }) {
  if (getStoreMode() === "postgres") {
    ...
    const orderForUpdate = resolvedOrderId ? await getOrderById(resolvedOrderId) : null;
    const effectiveStatus = orderForUpdate
      ? normalizePaymentStatusForAmount(orderForUpdate, status, amountBrl)
      : status;
    const review = buildPaymentReview(orderForUpdate, status, amountBrl, paymentId);
    const reviewIsNew = review && !isSamePaymentReview(orderForUpdate?.metadata?.paymentReview, review);
    ...
    if (orderId) {
      await updateOrderPaymentState(orderId, effectiveStatus, amountBrl, reviewIsNew ? review : null);
    }

    const updatedOrder = orderId ? await getOrderById(orderId) : null;

    if (updatedOrder && reviewIsNew) {
      await notifyInternalPaymentAlert(updatedOrder, review);
    }
  ...
  const reviewIsNew = review && !isSamePaymentReview(localOrder?.metadata?.paymentReview, review);
...
async function updateOrderPaymentState(orderId, paymentStatus, amountBrl, review = null) {
  const order = await getOrderById(orderId);
...
async function ensurePostgresSchema() {
  ...
  await query(`
    ...
    create table if not exists account_rate_limits (key text primary key, window_started_at timestamptz not null, attempts integer not null default 0);
