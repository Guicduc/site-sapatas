...
export async function createOrder(orderDraft, { verifiedAccount = null } = {}) {
  if (getStoreMode() === "postgres") {
    return createPostgresOrder(orderDraft, verifiedAccount);
  }

  return withLocalMutation(async () => {
    const store = await readLocalStore();
    const accountLink = resolveStoredAccountLink(store.customerAccounts, orderDraft, verifiedAccount);
    if (accountLink) orderDraft = markOrderEmailVerified(orderDraft);
...
export async function revokeCustomerAccountSession(accountId, tokenHash) {
  if (!accountId || !tokenHash) return;
  if (getStoreMode() === "postgres") {
    await query(`update customer_account_sessions set revoked_at = now() where account_id = $1 and token_hash = $2 and revoked_at is null`, [accountId, tokenHash]);
    return;
  }
  const store = await readLocalStore();
  const session = store.accountSessions.find((item) => item.accountId === accountId && item.tokenHash === tokenHash);
...
async function findActiveMercadoPagoPreference(client, orderId) {
  // Pix e boleto pendentes ja possuem provider_payment_id, mas o checkout_url
  // ainda aponta para a mesma preferencia e deve ser reutilizado.
...
function getVerifiedAccountCandidate(orderDraft, verifiedAccount) {
  const accountId = String(verifiedAccount?.accountId || verifiedAccount?.id || "").trim();
  const accountEmail = normalizeEmail(verifiedAccount?.email);
  const orderEmail = normalizeEmail(orderDraft.customer?.email);

  if (!accountId || !accountEmail || accountEmail !== orderEmail) return null;
  return { accountId, email: accountEmail };
}

function resolveStoredAccountLink(accounts, orderDraft, verifiedAccount) {
  const candidate = getVerifiedAccountCandidate(orderDraft, verifiedAccount);
  if (!candidate) return null;
  const account = accounts.find((item) =>
    item.id === candidate.accountId && normalizeEmail(item.email) === candidate.email
  );
  return account ? candidate : null;
}

async function resolvePostgresAccountLink(client, orderDraft, verifiedAccount) {
  const candidate = getVerifiedAccountCandidate(orderDraft, verifiedAccount);
  if (!candidate) return null;
  const result = await client.query(
    `select 1 from customer_accounts where id = $1 and lower(email) = $2`,
    [candidate.accountId, candidate.email]
  );
  return result.rows[0] ? candidate : null;
}

function markOrderEmailVerified(orderDraft) {
  return {
    ...orderDraft,
    metadata: {
      ...(orderDraft.metadata || {}),
      account: {
        ...(orderDraft.metadata?.account || {}),
...
async function createPostgresOrder(orderDraft, verifiedAccount) {
  await withTransaction(async (client) => {
    const accountLink = await resolvePostgresAccountLink(client, orderDraft, verifiedAccount);
    if (accountLink) orderDraft = markOrderEmailVerified(orderDraft);
    await client.query(
    ...
    if (accountLink) {
      await client.query(
        `insert into customer_account_orders (account_id, order_id)
         values ($1, $2)`,
        [accountLink.accountId, orderDraft.id]
      );
    }
  });
