...
export const productionStatusLabels = {
  ...
  [PRODUCTION_STATUS.WAITING_CAD]: "Aguardando CAD",
  [PRODUCTION_STATUS.QUEUED]: "Na fila",
  [PRODUCTION_STATUS.SCHEDULED]: "Programado",
  [PRODUCTION_STATUS.IN_PRODUCTION]: "Em producao",
...
function mergeProduction(current, patch = {}, now) {
  const status = validProductionStatus(patch.status) || current.status;
  return {
    ...current,
    status,
    priority: cleanText(patch.priority) || current.priority || "normal",
    scheduledDate: cleanText(patch.scheduledDate),
    machine: cleanText(patch.machine),
    operator: cleanText(patch.operator),
...
function mergeInvoice(current, patch = {}, now) {
  const status = validInvoiceStatus(patch.status) || current.status;
  return {
    ...current,
    status,
    mode: "manual",
    number: cleanText(patch.number),
    series: cleanText(patch.series),
    accessKey: cleanText(patch.accessKey),
    issuedAt: cleanText(patch.issuedAt) || (status === INVOICE_STATUS.MANUAL_ISSUED ? current.issuedAt || now : ""),
    notes: cleanText(patch.notes),
    updatedAt: now
  };
}
...
function mergeShipment(current, patch = {}, now) {
  const status = validShipmentStatus(patch.status) || current.status;
  return {
    ...current,
    status,
    carrier: cleanText(patch.carrier),
    trackingCode: cleanText(patch.trackingCode),
    shippedAt: cleanText(patch.shippedAt) || (status === SHIPMENT_STATUS.SHIPPED ? current.shippedAt || now : ""),
    deliveredAt: cleanText(patch.deliveredAt) || (status === SHIPMENT_STATUS.DELIVERED ? current.deliveredAt || now : ""),
    notes: cleanText(patch.notes),
    updatedAt: now
  };
}
...
function inferProductionStatus(order) {
  ...
  if ([ORDER_STATUS.CAD_PENDING, ORDER_STATUS.PAID_PENDING_REVIEW].includes(order?.status)) {
