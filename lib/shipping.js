...
export function shippingDimensionsForItem(item = {}) {
  ...
  const isUShoe = item.categorySlug === "sapata-u" && item.formatSlug === "u";
  const isInsertedPin = item.categorySlug === "sapata-pino" && item.formatSlug === "pino-inserido";
  const uOuterSpanMm = Number(values.diametro || 0) + Number(values.espessura || 0) * 2;
  const widthMm = firstPositive([
    isUShoe ? uOuterSpanMm : 0,
    isInsertedPin ? values.diametro : 0,
    values.tamanhoBaseX,
    values.largura,
  ...
  const heightMm = firstPositive([
    isUShoe ? uOuterSpanMm : 0,
    isInsertedPin ? values.alturaBase : 0,
    values.alturaBase,
    values.altura,
    values.alturaApoio,
    6
