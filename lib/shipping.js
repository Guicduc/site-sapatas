...
import { assertCartItemsLimits } from "./order-limits.js";
import { getProductManifestByRoute } from "./product-registry.js";
...
export async function quoteShippingForCheckout({ items = [], shippingAddress = {}, couponCode = "", customer = {} } = {}) {
  ...
  const promotion = await evaluatePromotion({ couponCode, subtotalBrl: itemsSubtotalBrl, shipping: quote, customer });
...
export function normalizeShippingItems(items = []) {
  ...
  return items.map((item) => {
    ...
    return {
      ...
      sku: format ? buildConfigurationSku(format, values, { color: item.color }) : cleanText(item.sku, 120),
      values,
      quantity,
...
export function shippingDimensionsForItem(item = {}) {
  ...
  const drawingType = format?.drawingType || getProductManifestByRoute(item.categorySlug, item.formatSlug)?.drawingType;

  if (item.categorySlug === "sapata-u") {
    const diameterMm = firstPositive([values.diametro]);
    const thicknessMm = firstPositive([values.espessura]);
    const lengthMm = firstPositive([values.comprimento]);
    const outerDiameterMm = diameterMm + thicknessMm * 2;

    return {
      widthCm: normalizeDimensionCm(outerDiameterMm / 10 + paddingCm),
      lengthCm: normalizeDimensionCm(lengthMm / 10 + paddingCm),
      heightCm: normalizeDimensionCm(diameterMm / 10 + paddingCm)
    };
  }

  if (drawingType === "tube-round-spherical") {
    const diameterMm = firstPositive([values.diametroBase]);
    const neckHeightMm = firstPositive([values.alturaPescoco]);

    return {
      widthCm: normalizeDimensionCm(diameterMm / 10 + paddingCm),
      lengthCm: normalizeDimensionCm(diameterMm / 10 + paddingCm),
      heightCm: normalizeDimensionCm((diameterMm / 2 + neckHeightMm) / 10 + paddingCm)
    };
  }
