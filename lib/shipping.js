...
async function quoteMelhorEnvio({ items, shippingAddress }) {
  ...
  return {
    ...
    shipment: summarizeShipment(payload.products),
    request: {
...
function summarizeShipment(products) {
  const totals = products.reduce(
    (summary, product) => {
      ...
      return {
        ...
        weightKg: summary.weightKg + Number(product.weight || 0) * quantity,
        insuranceValueBrl: summary.insuranceValueBrl + Number(product.insurance_value || 0) * quantity,
        maxWidthCm: Math.max(summary.maxWidthCm, Number(product.width || 0)),
        maxHeightCm: Math.max(summary.maxHeightCm, Number(product.height || 0)),
        maxLengthCm: Math.max(summary.maxLengthCm, Number(product.length || 0))
      };
    },
    {
      products: 0,
  ...
  return {
    ...totals,
    weightKg: roundMetric(totals.weightKg, 3),
    insuranceValueBrl: roundMoney(totals.insuranceValueBrl),
    maxWidthCm: roundMetric(totals.maxWidthCm, 2),
    maxHeightCm: roundMetric(totals.maxHeightCm, 2),
    maxLengthCm: roundMetric(totals.maxLengthCm, 2)
  };
}
