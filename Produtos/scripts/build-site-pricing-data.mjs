...
const previewOutputPath = path.join(repoRoot, "lib", "slicer-pricing-preview-data.js");
...
async function main() {
  ...
  const previewSource = [
    ...
    "// Public preview only: detailed slicer samples remain server-side.",
    `export const slicerPricingPreviewModels = ${JSON.stringify(pricingModels)};`,
    ""
  ].join("\n");

  await fs.writeFile(previewOutputPath, previewSource);
  console.log(`Preview compacto de precificacao: ${previewOutputPath}`);

  if (process.env.PRICING_PREVIEW_ONLY === "true") {
